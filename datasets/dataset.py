...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        num_workers: int = 0,
        seed: int = 42,
        train_shuffle: bool = False,   ### freezer 학습용으로 추가
    ):
        super().__init__()
    ...
    def train_dataloader(self):
        print(f"[INFO] train_shuffle={self.train_shuffle}, data_dir={self.data_dir}")   ### 확인용
        return DataLoader(
            self.train,
            ...
            pin_memory=True,
            shuffle=self.train_shuffle,   ### ← freezer dataset의 경우에만 
        )
