...
def main(
    ...
    # OOD masking 설정 주입 (학습 step에서 참조)
    classifier.ood_mask_prob = mask_prob
    classifier.ood_mask_num_segments = mask_num_segments    
    ...
    if is_train:
        ...
        if not os.path.exists("./ood_experiments/model/{}/".format(data)):
            os.makedirs("./ood_experiments/model/{}/".format(data))
        th.save(classifier.state_dict(), "./ood_experiments/model/{}/{}_classifier_{}_{}_maskaug".format(data, model_type, fold, seed))
    else:
        classifier.load_state_dict(th.load("./ood_experiments/model/{}/{}_classifier_{}_{}_maskaug".format(data, model_type, fold, seed), map_location=device))
    ...
    if "our_td" in explainers: 
        ...
        ###from attribution.explainers_td_rsd import OUR_TD ### rsd!
...
### 평가 부분은 main_preserve_td.py 에서 진행하므로 여기서는 삭제함! 

    if not os.path.exists("./results_ood_maskaug_attr/"): ### 실험에 따라 폴더 바꾸기!! _our=칼만스무더용 / _comp=fxc검산용 / _filter=칼만필터용 
        os.makedirs("./results_ood_maskaug_attr/")
    for key in attr.keys():
        result = attr[key]
        if isinstance(result, tuple): result = result[0]
        np.save('./results_ood_maskaug_attr/{}_{}_{}_result_{}_{}.npy'.format(data, model_type, key, fold, seed), result.detach().cpu().numpy())
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="OOD training: probability each sample gets masked",
    )
    parser.add_argument(
        "--mask_num_segments",
        type=int,
        default=5,
        help="OOD training: number of segments to zero out",
    )
    parser.add_argument(
        "--skip_train_timex",
        action='store_true'
...
if __name__ == "__main__":
    ...
    main(
        ...
        mask_num_segments=args.mask_num_segments, 
    )